  ...
  const DashboardView = () => {
    ...
    const runManualAnalysis = async () => {
      ...
      try {
        ...
        if (recCount > 0) {
          setSuccessMessage(`✅ Analysis completed! ${recCount} price recommendation${recCount > 1 ? 's' : ''} generated. Scroll down to review and apply. (${result.manualRemaining} manual analyses remaining today)`);
        } else {
          setSuccessMessage(`✅ Analysis completed! All prices are already optimized - no changes needed. (${result.manualRemaining} manual analyses remaining today)`);
        }
    ...
    const loadDashboardData = async () => {
      try {
        ...
        console.log('📊 Dashboard loaded:', {
          ...
          recommendations: recsData.recommendations?.length || 0
        });
      } catch (err) {
        setError(err.message);
      }
    ...
    const applyRecommendation = async (recommendationId, productId, newPrice) => {
      try {
        ...
        await loadDashboardData();
        setSuccessMessage('✅ Price updated successfully in your Shopify store!');
    ...
    const rejectRecommendation = async (recommendationId, productId) => {
      try {
        await api.call('/api/recommendations/reject', {
          method: 'POST',
        ...
        setError('Failed to dismiss recommendation: ' + err.message);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {activeTab === 'overview' && (
            <>
              {/* AI Profit Impact Card */}
              {stats && stats.totalAIProfit > 0 && (
                <div className="mb-8 bg-gradient-to-r from-green-600/20 via-emerald-600/20 to-teal-600/20 border-2 border-green-500/40 rounded-2xl p-8 shadow-2xl shadow-green-500/20">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-3">
                        ...
                        <div>
                          <h3 className="text-xl font-bold text-white">Total AI-Generated Profit</h3>
                          <p className="text-green-300 text-sm">Revenue increase from AI price optimizations</p>
                        </div>
                      </div>
                      <div className="ml-2">
                        <div className="text-6xl font-bold text-green-400 mb-2">
                          ${parseFloat(stats.totalAIProfit || 0).toFixed(2)}
                        </div>
                    ...
                    <div className="text-right">
                      <div className="p-4 bg-slate-900/50 rounded-xl mb-3">
                        <p className="text-gray-400 text-sm mb-1">Price Changes Applied</p>
                        <p className="text-white text-3xl font-bold">{stats.totalPriceChanges || 0}</p>
                      </div>
                      <div className="p-4 bg-slate-900/50 rounded-xl">
                        <p className="text-gray-400 text-sm mb-1">Products Optimized</p>
